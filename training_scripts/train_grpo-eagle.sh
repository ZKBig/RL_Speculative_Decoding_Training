...
export RUN_NAME=qwen3-4b-spec-eagle
export DATA_PATH=${WORKING_DIR}/data
export LOG_PATH=${WORKING_DIR}/logs
mkdir -p $LOG_PATH

# EAGLE draft model defaults
EAGLE_DRAFT_MODEL=${EAGLE_DRAFT_MODEL:-AngelSlim/Qwen3-4B_eagle3}
EAGLE_NUM_SPEC_TOKENS=${EAGLE_NUM_SPEC_TOKENS:-8}
EAGLE_NUM_SPEC_TOKENS_MIN=${EAGLE_NUM_SPEC_TOKENS_MIN:-1}
EAGLE_NUM_SPEC_TOKENS_MAX=${EAGLE_NUM_SPEC_TOKENS_MAX:-40}
EAGLE_METHOD=${EAGLE_METHOD:-eagle}

# Default values
PROJECT_NAME=grpo-qwen-4B-spec-eagle
...
python3 -m verl.trainer.main_ppo \
    ...
    +actor_rollout_ref.rollout.engine_kwargs.vllm.speculative_config.method=$EAGLE_METHOD \
    +actor_rollout_ref.rollout.engine_kwargs.vllm.speculative_config.model=$EAGLE_DRAFT_MODEL \
    +actor_rollout_ref.rollout.engine_kwargs.vllm.speculative_config.num_speculative_tokens=$EAGLE_NUM_SPEC_TOKENS \
    +actor_rollout_ref.rollout.engine_kwargs.vllm.speculative_config.draft_tensor_parallel_size=1 \
